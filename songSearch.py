...
try:
    print(response)
    with open(str(songName1) + '.json', 'w') as outfile:
...
songList = []
songIdList = []
with open(str(songName1)+'.json') as f:
    data = json.load(f)
...
try:
    print(response)
    with open(str(songName1)+ 'Id.json', 'w') as outfile:
        json.dump(response, outfile)
except:
        print("Doesn't work")


with open(str(songName1)+ 'Id.json') as f:
    data = json.load(f)
